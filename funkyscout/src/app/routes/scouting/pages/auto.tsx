...
const Auto = () => {
   ...
   useEffect(() => {
      ...
      if (canvas) {
         ...
         if (ctx) {
            ...
            const catmullRom = (p0: Note | { x: number, y: number }, p1: Note, p2: Note, p3: Note | { x: number, y: number }, t: number) => {
               const t2 = t * t;
               const t3 = t2 * t;
               return {
                  x: 0.5 * ((2 * p1.x) +
   ...
   const handleScore = () => {
      if (active) {
         setActive(false);
         AutoPath[AutoPath.length - 1].isClicked = true;
   ...
   const handleUndo = () => {
      ...
      if (undoActive && AutoPath[0].isClicked) {
         if (active) {
            ...
            console.log(AutoPath.pop());
            setActive(false);
         }
         else {
            AutoPath[AutoPath.length - 1].success = undefined;
            setActive(true);
               ...
               {alliance ? (
                  <svg id="svg" viewBox="0 0 327 233" fill="none" className="flipped">
                     <path d="M323.998 3.26295H213.758M3.38953 3.26295H115.292M115.292 3.26295L107.468 13.4348L157.923 96.7377H172.594L221.094 13.4348L213.758 3.26295M115.292 3.26295H213.758M3.38953 104.59H19.821V158.971M19.821 229.978V158.971M19.821 158.971H292.309M65.7899 229.978L87.6985 196.528H127.408L146.578 229.978"
                        stroke="#B54545" strokeWidth="6" strokeLinecap="round" strokeLinejoin="round" fill="none"
                     />
