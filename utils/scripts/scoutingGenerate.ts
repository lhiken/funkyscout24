...
const getScouter = async (event: string) => {
   ...
   if (s) {
      for (let i = 0; i < s.length; i++) {
         scouters.push([s[i][0], Math.max(Math.min(i - s.length + 0), 0), 0]);
      }
   } else return;
...
const findScouter = (scouters: [string, number, number][], scout: number) => {
   ...
   scouters = scouters.sort((a, b) => b[1] - a[1]);
   ...
   for (let i = 0; i < scouters.length; i++) {
      if (
         scouters[i][1] >= 0 && scouters[i][1] < scout &&
         assignScouter.length <= 6
      ) {
...
const assignScouter = async (event: string, scout: number) => {
   ...
   if (event_data && scouters) {
      ...
      console.log(
         possibleBreak + " possible break after " + scout + " scoutingship",
      );
      ...
      for (let i = 0; i < event_data.length; i++) {
         ...
         if (typeof match == "number" && typeof team == "number") {
            const { data, error } = await supabase
               .from("event_data")
               .update({
                  author: selectedScouters[i % selectedScouters.length][0],
               })
               .eq("event", event!)
               .eq("match", match)
               .eq("team", team);
         ...
         for (let k = 0; k < scouters.length; k++) {
            if (
               scouters[k][0] ===
                  selectedScouters[i % selectedScouters.length][0]
            ) {
               scouters[k][1]++;
               scouters[k][2]++;
