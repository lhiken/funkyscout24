...
async function SetupEvent(event_code: string) {
      ...
      for (let team of data) {
         ...
         try {
            const { data: team_data, error } = await supabase
               .from('team_data')
               .insert([
                  { event: event_code, team: team.team_number, avg_score: 0, avg_amp: 0, avg_speaker: 0, failures: 0, defense: 0 },
               ])
               .select()
            ...
            if ((error) != null) {
               process.stdout.clearLine(-1);
               process.stdout.cursorTo(0);
               process.stdout.write(`| An error occured; upload unsuccessful.\n`)
               teamError = error;
               break;
            }
         } catch {
   ...
   fetch(`https://www.thebluealliance.com/api/v3/event/${event_code}/teams/simple`, {
      headers: {
...
async function setupMatches(event_code: string) {
   ...
   const updateMatches = async (data: any[]) => {
      ...
      for (let match of data) {
         if (match.comp_level == "qm") {
            ...
            for (let team of redAlliance.team_keys) {
               process.stdout.clearLine(-1);
               process.stdout.cursorTo(0);
               process.stdout.write(`| Attempting update: qm${match_num}r, Team ${team}`);
               team = team.slice(3);
               ...
               try {
                  const { data: event_data, error } = await supabase
                     .from('event_data')
                     .insert([
                        { event: event_code, match: match_num, team: team, alliance: true },
                     ])
                     .select()
            ...
            for (let team of blueAlliance.team_keys) {
               process.stdout.clearLine(-1);
               process.stdout.cursorTo(0);
               process.stdout.write(`| Attempting update: qm${match_num}b, Team ${team}`);
               ...
               try {
                  const { data: event_data, error } = await supabase
                     .from('event_data')
                     .insert([
                        { event: event_code, match: match_num, team: team, alliance: false },
                     ])
                     .select()
      ...
      console.log(`| Operation completed in ${Math.round(endTime-startTime)/1000}s.`)
   ...
   fetch(`https://www.thebluealliance.com/api/v3/event/${event_code}/matches/simple`, {
      ...
      .then(
         data => {
            console.log(`| Received ${data.length} matches, expecting ~${(data.length - 16) * 6} updates.\n`)
            updateMatches(data);
         })
      .catch(error => {
         console.error(`Error occured with ${error}`)
